package com.samuelberrien.odyspace.utils.widget;
...
public class ExpandButton extends LinearLayout {
	...
	private void init(Context context, final Runnable onConfirm) {
		...
		mainButton.setBackground(ContextCompat.getDrawable(context, R.drawable.strokes));

		confirmButton = new Button(context);
		confirmButton.setGravity(Gravity.CENTER);
		confirmButton.setBackground(ContextCompat.getDrawable(context, R.drawable.confirm));
		confirmButton.setText("GO");
		confirmButton.setTextColor(ContextCompat.getColor(getContext(), R.color.colorPrimaryDark));
