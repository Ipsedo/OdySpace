...
public class Fire {
    ...
    public static void fire(ObjModelMtl rocketModel, List<BaseItem> rockets, Fire.Type type, float[] position, float[] originalSpeedVec, float[] rotationMatrix, float maxSpeed) {
        switch (type) {
            case SIMPLE_FIRE:
                rockets.add(new Rocket(rocketModel, position, originalSpeedVec, new float[]{0f, 0f, 0f}, rotationMatrix, maxSpeed, 1f));
                break;
            case QUINT_FIRE:
                rockets.add(new Rocket(rocketModel, position.clone(), originalSpeedVec.clone(), new float[]{0f, 0f, 0f}, rotationMatrix.clone(), maxSpeed, 1f));

                float[] tmpMat = new float[16];
                Matrix.setRotateM(tmpMat, 0, 2.5f, 1f, 0f, 0f);
                float[] res = new float[16];
                Matrix.multiplyMM(res, 0, rotationMatrix, 0, tmpMat, 0);
                rockets.add(new Rocket(rocketModel, position.clone(), originalSpeedVec.clone(), new float[]{0f, 0f, 0f}, res.clone(), maxSpeed, 1f));
                ...
                break;
            case SIMPLE_BOMB:
                rockets.add(new Rocket(rocketModel, position, originalSpeedVec, new float[]{0f, 0f, 0f}, rotationMatrix, maxSpeed, 2.5f));
                break;
        }
    }
}
