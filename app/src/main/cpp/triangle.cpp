...
/*extern "C" {
	...
	trianglewrapper(JNIEnv *env, jobject thiz, float p0[3], float p1[3], float p2[3], float q0[3], float q1[3], float q2[3])
	{
		return triangle_intersection(p0, p1, p2, q0, q1, q2);
	}
}*/

extern "C" {
JNIEXPORT jboolean JNICALL
Java_com_samuelberrien_odyspace_objects_BaseItem_triangleIntersection(JNIEnv *env,
                                                                      jobject instance,
                                                                      jfloatArray p0,
                                                                      jfloatArray p1,
                                                                      jfloatArray p2,
    ...
    // TODO
    if (triangle_intersection(env->GetFloatArrayElements(p0, 0), env->GetFloatArrayElements(p1, 0),
                              env->GetFloatArrayElements(p2, 0), env->GetFloatArrayElements(q0, 0),
                              env->GetFloatArrayElements(q1, 0),
                              env->GetFloatArrayElements(q2, 0))) {
        return JNI_TRUE;
    }
    return JNI_FALSE;
}
