...
int tr_tri_intersect3D(float *C1, float *P1, float *P2,
    ...
    if ((dq1 == 0) && (dq2 == 0)) {
        if (dr != 0) return 0;  // triangles are on parallel planes
            ...
            //as input.
            myVpV(C2, C1, P1);myVpV(C3, C1, P2);myVpV(D2, D1, Q1);myVpV(D3, D1, Q2);
            CROSS(N1, P1, P2);
...
inline void
computeEdges(float v0[3], float v1[3], const float p0[3], const float p1[3], const float p2[3]) {
    v0[0] = p1[0] - p0[0];
    v0[1] = p1[1] - p0[1];
    v0[2] = p1[2] - p0[2];
    v1[0] = p2[0] - p0[0];
