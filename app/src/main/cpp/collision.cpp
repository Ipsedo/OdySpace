...
bool areCollided(float* items1Points, int nbEl1, float* item1Model, float* items2Points, int nbEl2, float* item2Model) {
    ...
    for(int i = 0; i < nbEl1 / 9; i++){
        ...
        glm::vec4 u1 = glm::vec4(items1Points[currI + 3], items1Points[currI + 4], items1Points[currI + 5], 1.0);
        glm::vec4 u2 = glm::vec4(items1Points[currI + 6], items1Points[currI + 7], items1Points[currI + 8], 1.0);
    ...
    for(int i = 0; i < nbEl2 / 9; i++){
        int currI = i * 9;
        glm::vec4 vvec0 = glm::vec4(items2Points[currI + 0], items2Points[currI + 1], items2Points[currI + 2], 1.0);
        glm::vec4 vvec1 = glm::vec4(items2Points[currI + 3], items2Points[currI + 4], items2Points[currI + 5], 1.0);
        glm::vec4 vvec2 = glm::vec4(items2Points[currI + 6], items2Points[currI + 7], items2Points[currI + 8], 1.0);
        ...
        for(int j = 0; j < nbEl1 / 9; j++){
            int currJ = j * 9;
            double u0[] = {items1Points[currJ + 0], items1Points[currJ + 1], items1Points[currJ + 2]};
            double u1[] = {items1Points[currJ + 3], items1Points[currJ + 4], items1Points[currJ + 5]};
            double u2[] = {items1Points[currJ + 6], items1Points[currJ + 7], items1Points[currJ + 8]};
...
Java_com_samuelberrien_odyspace_objects_BaseItem_areCollided(JNIEnv *env, jobject instance,
    ...
    bool res = areCollided(env->GetFloatArrayElements(mPointItem1_, 0), env->GetArrayLength(mPointItem1_), env->GetFloatArrayElements(mModelMatrix1_ ,0), env->GetFloatArrayElements(mPointItem2_, 0), env->GetArrayLength(mPointItem2_), env->GetFloatArrayElements(mModelMatrix2_ ,0));
